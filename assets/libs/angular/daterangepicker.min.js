(function(a, b) {
  ...
  if (typeof s.startDate === "undefined" && typeof s.endDate === "undefined") {
   if (d(this.element).is("input[type=text]")) {
    var i = d(this.element).val(),
     r = i.split(this.locale.separator);
    h = l = null;
    if (r.length == 2) {
 ...
 a.prototype = {
  ...
  move: function() {
   ...
   if (this.opens == "left") {
    ...
    if (this.opens == "center") {
     ...
     this.container.css({
      top: h,
      left: this.element.offset().left - f.left,
      right: "auto"
     });
  ...
  monthOrYearChanged: function(i) {
   var k = d(i.target).closest(".calendar").hasClass("left"),
    j = k ? "left" : "right",
    h = this.container.find(".calendar." + j);
  ...
  timeChanged: function(l) {
   var f = d(l.target).closest(".calendar"),
    n = f.hasClass("left");
